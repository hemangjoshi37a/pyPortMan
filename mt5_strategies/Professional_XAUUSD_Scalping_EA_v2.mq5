//+------------------------------------------------------------------+
//|              Professional_XAUUSD_Scalping_EA_v2.mq5              |
//|           Professional XAU/USD Scalping Bot - DEBUG VERSION        |
//|           More lenient parameters with detailed logging           |
//+------------------------------------------------------------------+
#property copyright "Professional XAU/USD Scalping Bot v2"
#property version   "2.00"
#property strict

//+------------------------------------------------------------------+
//| INPUT PARAMETERS - MORE LENIENT FOR TESTING                       |
//+------------------------------------------------------------------+
...
input int      ADX_Threshold = 20;    // ADX Threshold (LOWERED from 25)

// === Scalping Entry Settings ===
input group "=== Scalping Entry Settings ==="
input int      Min_Signal_Score = 25;  // Minimum Signal Score (LOWERED from 45)
...
input int      Max_Spread = 50;               // Max Spread (INCREASED from 30)
input bool     Close_Opposite = true;         // Close Opposite on Signal
input int      Max_Trades_Per_Hour = 5;       // Max Trades Per Hour (INCREASED from 3)
input int      Min_Candles_Between_Trades = 2; // Min Candles Between (DECREASED from 3)

// === Debug Settings ===
input group "=== Debug Settings ==="
input bool     Enable_Debug_Log = true;       // Enable Debug Logging
input bool     Show_Signal_Breakdown = true;  // Show Signal Score Breakdown
...
// Debug Counters
int debugTotalBars = 0;
int debugBlockedBySession = 0;
int debugBlockedByMinCandles = 0;
int debugBlockedByMaxTradesHour = 0;
int debugBuySignals = 0;
int debugSellSignals = 0;
int debugHighestBuyScore = 0;
int debugHighestSellScore = 0;
...
int OnInit()
{
    ...
    Print("========================================");
    Print("Professional XAU/USD Scalping EA v2");
    Print("========================================");
    ...
    Print("Max Spread: ", Max_Spread, " points");
    Print("Max Trades Per Hour: ", Max_Trades_Per_Hour);
    Print("Min Candles Between: ", Min_Candles_Between_Trades);
    Print("========================================");
...
void OnDeinit(const int reason)
{
    ...
    Print("========================================");
    Print("Total bars analyzed: ", debugTotalBars);
    Print("Blocked by session: ", debugBlockedBySession);
    Print("Blocked by min candles: ", debugBlockedByMinCandles);
    Print("Blocked by max trades/hour: ", debugBlockedByMaxTradesHour);
    Print("Buy signals generated: ", debugBuySignals);
    Print("Sell signals generated: ", debugSellSignals);
    Print("Highest buy score: ", debugHighestBuyScore);
    Print("Highest sell score: ", debugHighestSellScore);
    Print("========================================");
    Print("Professional XAU/USD Scalping EA v2 stopped. Reason: ", reason);
}
...
void CheckForSignals()
{
    ...
    if(currentSignal.buyScore > debugHighestBuyScore)
        debugHighestBuyScore = currentSignal.buyScore;
    if(currentSignal.sellScore > debugHighestSellScore)
        debugHighestSellScore = currentSignal.sellScore;

    // Debug logging every 100 bars
    if(Enable_Debug_Log && debugTotalBars % 100 == 0)
    {
        Print("DEBUG: Bar #", debugTotalBars, " | Buy Score: ", currentSignal.buyScore, " | Sell Score: ", currentSignal.sellScore, " | Required: ", Min_Signal_Score);
    }
...
void OpenBuyPosition()
{
    ...
    request.comment = "PRO_SCALPING_BUY_v2";

    if(OrderSend(request, result))
    {
        Print("✅ BUY opened at ", ask, " | Lot: ", lot, " | SL: ", sl, " | TP: ", tp4);
        Print("   Signal Score: ", currentSignal.buyScore, " | Required: ", Min_Signal_Score);

        if(Show_Signal_Breakdown)
        {
            Print("   Breakdown:");
            if(msH4.isUptrend) Print("     - H4 Uptrend: +8");
            if(msH1.isUptrend) Print("     - H1 Uptrend: +7");
            if(msM15.isUptrend) Print("     - M15 Uptrend: +5");
            if(ask > emaFastBuffer[0] && emaFastBuffer[0] > emaSlowBuffer[0]) Print("     - EMA Alignment: +5");
            if(adxBuffer[0] > ADX_Threshold) Print("     - ADX Strong: +10");
            if(rsiBuffer[0] < 50) Print("     - RSI Bullish: +5");
            if(IsBullishEngulfing()) Print("     - Bullish Engulfing: +6");
            if(IsBullishPinbar()) Print("     - Bullish Pinbar: +6");
        }
...
void OpenSellPosition()
{
    ...
    request.deviation = 10;
    request.magic = Magic_Number;
    request.comment = "PRO_SCALPING_SELL_v2";

    if(OrderSend(request, result))
    {
        Print("✅ SELL opened at ", bid, " | Lot: ", lot, " | SL: ", sl, " | TP: ", tp4);
        Print("   Signal Score: ", currentSignal.sellScore, " | Required: ", Min_Signal_Score);

        if(Show_Signal_Breakdown)
        {
            Print("   Breakdown:");
            if(msH4.isDowntrend) Print("     - H4 Downtrend: +8");
            if(msH1.isDowntrend) Print("     - H1 Downtrend: +7");
            if(msM15.isDowntrend) Print("     - M15 Downtrend: +5");
            if(bid < emaFastBuffer[0] && emaFastBuffer[0] < emaSlowBuffer[0]) Print("     - EMA Alignment: +5");
            if(adxBuffer[0] > ADX_Threshold) Print("     - ADX Strong: +10");
            if(rsiBuffer[0] > 50) Print("     - RSI Bearish: +5");
            if(IsBearishEngulfing()) Print("     - Bearish Engulfing: +6");
            if(IsBearishPinbar()) Print("     - Bearish Pinbar: +6");
        }
...
bool CheckWeeklyLossLimit()
{
    ...
    if(dt.day_of_week < resetDt.day_of_week ||
       (dt.day_of_week == 0 && resetDt.day_of_week > 0) ||
       (dt.day != resetDt.day && dt.mon != resetDt.mon))
    {
        weeklyStartBalance = AccountInfoDouble(ACCOUNT_BALANCE);
        weeklyResetTime = TimeCurrent();
    }
